...
recordRoutes.route("/record/add").post(async function (req, response) {
    try {
        ...
        const res = await db_connect.collection("records").insertOne(myobj);
        response.json(res);
    } catch (err) {
        throw err;
    }
...
recordRoutes.route("/update/:id").post(async function (req, response) {
    try {
        ...
        const res = await db_connect.collection("records").updateOne(myquery, newvalues);
...
// convert the rest of the routes to be the same kind as the one above...
// forum post -> https://www.mongodb.com/community/forums/t/mern-stack-tutorial-guide-doesnt-work/222971/9
// documentation -> https://www.mongodb.com/docs/drivers/node/v3.6/fundamentals/promises/
